    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-4 rounded-lg shadow-lg">
                <p className="text-lg text-gray-800">Are you sure you want to delete {itemName}?</p>
...
function View() {
    ...
    return (
            ...
            <div className="max-w-md mx-auto my-4 p-4">
                ...
                {favPackage.length > 0 ? (
                    <div className='flex flex-col items-center'>
                        <div className="w-full max-w-3xl p-6 bg-white  border-gray-400 ">
                            <table className="min-w-full bg-white border border-gray-300">
                                <thead>
                                    <tr>
                                        <th className="py-2 px-4 border-b border-gray-300">Package Name</th>
                                        <th className="py-2 px-4 border-b border-gray-300 border-l">Actions</th> {/* Added border-l class for vertical line */}
                                    </tr>
                                </thead>
                                <tbody>
                                    {favPackage.map(pkg => (
                                        <tr key={pkg.name}>
                                            <td className="py-2 px-4 border-b border-gray-300">{pkg.name}</td>
                                            <td className="py-2 px-4 border-b border-gray-300 border-l"> {/* Added border-l class for vertical line */}
                                                <div className='flex justify-around'>
                                                    <button className='text-blue-500 hover:text-blue-700' onClick={() => alert(`Reason:  ${pkg.reason}`)}>
                                                        <svg fill="#000000" className='h-5 w-5' viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" enableBackground="new 0 0 42 42" xmlSpace="preserve">
        ...
        C19.4,32.6,22.4,32.6,23.4,32.4z M11.1,20.7c0-5.2,4.4-9.4,9.9-9.4s9.9,4.2,9.9,9.4S26.5,30,21,30S11.1,25.8,11.1,20.7z" />
                                                    ...
                                                    <button className="text-yellow-500 hover:text-yellow-700" onClick={() => handleEdit(pkg)}>
                                                    ...
                                                    <button className="text-red-500 hover:text-red-700" onClick={() => handleDelete(pkg.name)}>
                                                        <svg className='h-5 w-5' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <p className='text-center'>No favorite packages added yet.</p>
                )}
            </div>
        </>
    );
